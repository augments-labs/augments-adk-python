...
augments serve --agent app:agent --host 0.0.0.0 --port 8000
...
augments deploy init --agent app:agent --image my-agent:latest

# Or a full target set (Dockerfile + Kubernetes manifests):
augments deploy init --target k8s --agent app:agent --image my-agent:latest \
  --env-key OPENAI_API_KEY
```

The generated `requirements.txt` installs `augments-adk` from PyPI. Add
...
augments deploy build --agent app:agent --image my-agent:latest --push

# GCP Cloud Run (Cloud Build — no local docker needed):
augments deploy cloud-run --agent app:agent \
  --image gcr.io/PROJECT/my-agent --project PROJECT --region REGION

# Kubernetes (any cluster):
augments deploy k8s --agent app:agent --image my-agent:latest

# AWS ECS Fargate (push the image to ECR first):
augments deploy ecs --agent app:agent \
