"""Augments Agents ADK - A lightweight, provider-agnostic Python framework for building multi-agent workflows.
...
Example (streaming):
    from augments.adk import Agent, Runner, RunItemType
...
Example (with LLMConfig):
    from augments.adk import Agent, Runner, LLMConfig
...
_logger = _logging.getLogger("augments.adk")
...
def setup_logging() -> None:
    ...
    event stream (see ``augments.adk.verbose``). When the config is
    ...
    Users can reconfigure after import:
        logging.getLogger("augments.adk").setLevel(logging.WARNING)
    """
    # src/augments/adk/__init__.py → 4 levels up to project root
        ...
        try:
            ...
            if "filename" in file_cfg:
                ...
                if reusable is not None:
                    ...
                    file_cfg["filename"] = str(log_dir / f"augments_adk.{timestamp}.log")
...
from augments.adk.agents import (
    Agent,
...
from augments.adk.session import Session, SessionSettings, SQLiteMultiSessions
...
from augments.adk.tools import DocumentSearchTool, FunctionTool, MemoryTool
