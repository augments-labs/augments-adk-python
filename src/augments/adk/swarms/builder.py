...
Design goals (mirrors ``augments.adk.graphs.builder``):
...
1. **Readability first.** A swarm definition should read top-to-bottom
   ...
   - AutoGen: ``Swarm([travel_agent, refunder], termination_condition=HandoffTermination("user") | TextMentionTermination("TERMINATE"))``
     ...
     ``handoffs=[...]`` lists.
   - Augments (this file): ``Swarm.new("support").members(triage, refunds).entry("triage").llm_handoff().terminate_on(ExplicitDoneTermination() | MaxTurnsTermination(12)).compile()``.
...
if TYPE_CHECKING:
    ...
    from augments.adk.swarms.policy import SwarmExtraToolsFn, SwarmSelector
