...
from augments.adk.graphs.interrupt import NESTED_AGENT_TOOL_APPROVAL_KIND, Interrupt, NestedAgentInterrupt
from augments.adk.run.state import RunState
from augments.adk.swarms.yield_signal import SwarmDone, SwarmHandoff, SwarmYieldSignal
...
class SwarmState[TContext]:
    ...
    ``augments.swarm.id`` attribute appears on suspend-side and resume-side
    ...
    ``augments.swarm.turn.resume_attempt``. Empty dict for fresh runs."""
    ...
    def to_dict(self) -> SwarmStateDict:
        ...
        """
        from augments.adk.types.items.items import ItemHelpers  # local to avoid cycle
...
Mirrors ``augments.adk.graphs.state._VALID_STATUS_VALUES``. Prevents
