...
- ``StructuredRoutingPolicy`` — the Augments differentiator. Wraps a
...
from augments.adk.swarms.yield_signal import SWARM_DONE_TOOL_NAME, SwarmYieldSignal
...
def _build_transfer_tool(target_name: str, description: str | None = None) -> FunctionTool:
    ...
    Uses the same name prefix as ``augments.adk.handoffs`` —
...
# ---------------------------------------------------------------------------
# StructuredRoutingPolicy — the Augments differentiator
# ---------------------------------------------------------------------------
...
class StructuredRoutingPolicy(SwarmPolicy[TContext]):
    ...
    This is the Augments differentiator: full type safety, no routing
