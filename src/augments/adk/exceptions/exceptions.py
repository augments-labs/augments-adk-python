...
if TYPE_CHECKING:
    ...
    from augments.adk.status.types import AgentQuota
    from augments.adk.tools.deferred_tool import DeferredToolRequests


class AugmentsError(Exception):
    """Base exception for all Augments ADK errors."""
    ...
    def __init__(self, message: str | None = None):
        ...
        """
        self.message = message or "An error occurred in the Augments ADK."
...
class MemoryExtractionError(AugmentsError):
...
class GuardrailTripwireTriggered(AugmentsError):
...
class ToolGuardrailTripwireTriggered(AugmentsError):
    """
...
class MaxTurnsExceeded(AugmentsError):
    """
...
class UsageLimitExceeded(AugmentsError):
    """
...
class ToolTimeoutError(AugmentsError):
...
class AgentToolDeferral(AugmentsError):
...
class HandoffRejection(AugmentsError):
...
class ToolDependencyError(AugmentsError):
...
class DocumentLoadError(AugmentsError):
...
class UnsupportedDocumentSourceError(AugmentsError):
...
class ToolsetNameConflictError(AugmentsError):
...
class TracingDependencyError(AugmentsError):
    """Raised when the OpenTelemetry extra is required but not installed.

    The OTel bridge (``augments.adk.tracing.otel``) performs a soft
    ...
    def __init__(self, missing: str, message: str | None = None) -> None:
        ...
        super().__init__(
            ...
            or (
                ...
                "Install the optional extra: pip install 'augments-adk[otel]'"
            )
        )


class QuotaExceeded(AugmentsError):
...
class TenantBudgetExceeded(AugmentsError):
...
class ToolNotPermittedForTenant(AugmentsError):
...
class NoRoutingCandidateError(AugmentsError):
    """Raised when every routing candidate failed (or the list was empty)."""


class ModelRefusalError(AugmentsError):
...
class SandboxError(AugmentsError):
...
class CheckpointConflictError(AugmentsError):
...
class SessionAppendConflictError(AugmentsError):
