...
The `augments` console script (`[project.scripts]` → `augments.adk.cli:main`,
also `python -m augments.adk.cli`). Click-based; one module per command.
...
| `__main__.py` | `python -m augments.adk.cli` entry |
...
| `run.py` | `augments run` + shared `build_run_config`, `open_session`, `echo_stream`, `target_app_name` |
| `chat.py` | `augments chat` — CLI-owned REPL (NOT a `run_demo_loop` wrapper: the demo loop has no `session=`) |
| `validate.py` | `augments validate` (`--resolve` assembles refs) |
| `schema.py` | `augments schema agent\|node\|topology` |
| `sessions.py` | `augments sessions list\|show\|delete` |
| `new.py` + `templates.py` | `augments new` scaffolding (string.Template, `$name`; schema files written beside the config) |
| `serve.py` | `augments serve` — REST + health by default via `serving.build_app`, A2A added with `--card`, run under uvicorn; behind the `serve` extra |

## Key decisions

| Decision | Rationale |
|---|---|
| click is a core dep | Zero transitive deps on POSIX; the entry point must work on a base install (first-touch UX: `pip install augments-adk && augments new`). The upstream mistake to avoid was making the *web/server stack* core, not click. |
...
| Heavy framework imports live inside command bodies | Fast `--help`; the parent `augments.adk` import is already paid, but command modules must not add more at import time. |
...
| uvicorn imported only inside `serve` | The framework deliberately doesn't own the ASGI runtime; the `serve` extra (`augments-adk[server,a2a]` + uvicorn) gates it with a guiding message. `serve` routes through `serving.build_app`, never importing the app stack at module load. |
