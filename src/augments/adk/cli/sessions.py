"""``augments sessions`` — inspect and prune SQLite session stores.

Operates on the same stores ``augments run --session-db`` and
``augments chat --session-db`` write. The store is app-scoped, so every
