"""``augments chat`` — interactive multi-turn REPL against a single agent.
...
from augments.adk.cli.options import run_options, session_options, target_options
from augments.adk.cli.run import build_run_config, echo_stream, open_session
...
def chat(
    ...
    if not isinstance(executable, Agent):
        raise click.UsageError("chat drives a single agent; use 'augments run' for swarms and graphs.")
