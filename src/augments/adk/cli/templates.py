"""Project templates for ``augments new``.
    ...
    """\
{
  ...
  "name": "$name",
  "description": "Starter agent scaffolded by the Augments CLI.",
...
A starter Augments ADK agent project.

```bash
augments validate agent.json            # schema-check (no tokens spent)
augments validate --resolve agent.json  # also import the tool references
augments run agent.json "What time is it?"
...
A starter Augments ADK multi-agent topology (triage → expert handoff).

```bash
augments validate topology.json             # schema-check (no tokens spent)
augments run topology.json "Explain BSP supersteps"
```
...
#   augments run agent.json --env-file .env "hello"
# ANTHROPIC_API_KEY=
# OPENAI_API_KEY=
# GEMINI_API_KEY=
"""
...
def render_project(kind: str, name: str) -> dict[str, str]:
    ...
    """
    from augments.adk.config.schema import dump_agent_config_schema, dump_topology_config_schema
