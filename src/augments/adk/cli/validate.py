"""``augments validate`` — check a config file without spending a token.
...
schemas, providers) actually import — the same path ``augments run`` takes,
minus execution.
"""
...
from augments.adk.cli.loading import ConfigKind, detect_config_kind
...
def validate(config: Path, kind: ConfigKind | None, resolve_refs: bool) -> None:
    ...
    from augments.adk.config.loader import read_config_document
...
def _schema_check(path: Path, data: dict[str, Any], kind: ConfigKind) -> str:
    ...
    if kind == "topology":
        from augments.adk.types.config.topology_config import TopologyConfig
...
def _assemble(path: Path, kind: ConfigKind) -> str:
    ...
    if kind == "topology":
        from augments.adk.config.topology import load_topology

        topology = load_topology(path)
        names = ", ".join(sorted(topology.agents))
        return f"topology ({len(topology.agents)} agents: {names}) resolved"

    from augments.adk.config.loader import load_agent
