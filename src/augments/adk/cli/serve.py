"""``augments serve`` — expose a local Agent over HTTP.

Serves a single Agent through the framework's serving layer
(``augments.adk.serving.build_app``) under uvicorn. By default it
...
For containers, bind all interfaces and read the platform port::
    ...
    augments serve --agent app:agent --host 0.0.0.0 --port "$PORT"
...
from augments.adk.cli.errors import framework_errors
from augments.adk.cli.loading import load_env_file, primary_executable, resolve_target
from augments.adk.cli.options import target_options
...
if TYPE_CHECKING:
    ...
    from augments.adk.a2a.server import A2AServer
    from augments.adk.agents.agent import Agent
    from augments.adk.session.multi_sessions import MultiSessions
    from augments.adk.types.session.store import SessionStore
...
SERVE_INSTALL_HINT = "augments serve requires the serve extra. Install with: pip install 'augments-adk[serve]'"
A2A_INSTALL_HINT = "the A2A surface requires the a2a extra. Install with: pip install 'augments-adk[a2a]'"
A2A_PG_HINT = "--task-dsn requires the a2a-postgres extra. Install with: pip install 'augments-adk[a2a-postgres]'"
SESSION_PG_HINT = (
    "--session-dsn requires the session-postgres extra. Install with: pip install 'augments-adk[session-postgres]'"
)
