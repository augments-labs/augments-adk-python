"""``augments deploy`` — generate deployment artifacts and ship the agent.
...
from augments.adk.deploy.commands import DeployCommandFailed, DeployToolMissing, SubprocessRunner
from augments.adk.deploy.context import DeployContext
from augments.adk.deploy.helm_chart import split_image_reference
from augments.adk.deploy.targets import TARGETS
...
def context_options[F: Callable[..., object]](f: F) -> F:
    ...
    f = click.option("--image", default="augments-agent:latest", show_default=True, help="Container image name[:tag].")(
        f
    )
    f = click.option(
    ...
    f = click.option(
        "--extras", default="serve,a2a", show_default=True, help="augments-adk extras installed in the image."
    )(f)
    f = click.option(
        "--env-key",
        "env_keys",
...
def _context(
    ...
    Args:
        ...
        port: Container port.
        extras: ``augments-adk`` extras to install.
...
def init(
    ...
    click.echo("")
    click.echo("Next: make requirements.txt install augments-adk, then build the image:")
    click.echo(f"  augments deploy build --agent {agent_ref} --image {ctx.image}")
