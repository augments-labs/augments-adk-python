"""``augments run`` — execute an agent, swarm, graph, or topology once.
...
from augments.adk.cli.loading import load_env_file, primary_executable, reconcile_positionals, resolve_target
from augments.adk.cli.options import output_option, run_options, session_options, target_options
...
def _enable_tracing() -> None:
    ...
    from augments.adk.tracing import set_tracer, setup_otel

    if setup_otel is None:
        raise click.UsageError(
            "--trace requires the OpenTelemetry extra. Install with: pip install 'augments-adk[otel]'"
        )
...
async def _execute(
    ...
    *,
    ...
    from augments.adk.agents.agent import Agent
    from augments.adk.run.config import DEFAULT_MAX_TURNS
