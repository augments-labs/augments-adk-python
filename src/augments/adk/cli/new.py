"""``augments new`` — scaffold a runnable agent project.
...
example, and a README — a project ``augments validate --resolve`` passes
with no edits.
"""
...
from augments.adk.cli.templates import render_project
...
def new(name: str, kind: str, parent_dir: Path) -> None:
    ...
    click.echo(f"  augments validate {target_dir / config_name}")
    click.echo(f'  augments run {target_dir / config_name} "hello"')
