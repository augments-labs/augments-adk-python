...
Thin layer over ``augments.adk.llms.retry.call_with_retry`` that
supplies the OpenAI SDK's exception classifier. The generic loop
(backoff, jitter, budget accounting) lives in
``augments.adk.llms.retry`` — this module only translates the
...
from augments.adk.llms.retry import call_with_retry as _generic_call_with_retry
...
async def call_with_retry[T](
    ...
    *,
    ...
    See ``augments.adk.llms.retry.call_with_retry`` for the loop
