...
class GitRepoLazySkillSource(BaseModel):
    ...
    def _ensure_clone(self) -> Path:
        ...
        clone_dir = Path(tempfile.mkdtemp(prefix="augments-skills-"))
...
class SkillsCapability(SandboxCapability):
    ...
    def process_manifest(self, manifest: Manifest) -> Manifest:
        ...
        from augments.adk.types.sandbox.manifest import Manifest as _Manifest
    ...
    def tools(self) -> list[FunctionTool]:
        if self.lazy_from is None or self.session is None:
            return []
        from augments.adk.sandbox.tools.load_skill_tool import make_load_skill_tool
