...
``apply_*_to_k8s_pod`` helpers in ``augments.adk.sandbox.policy``.
"""
...
def _require_k8s() -> None:
    try:
        ...
        raise UnsupportedSandboxClientError(
            "K8sPodSandboxClient requires the optional 'kubernetes' package: pip install 'augments-adk[sandbox-k8s]'",
        ) from exc
...
def _build_pod_manifest(options: K8sSandboxClientOptions, pod_name: str) -> dict[str, Any]:
    ...
    labels: dict[str, str] = {
        "app.kubernetes.io/managed-by": "augments-adk",
        "augments.adk.io/role": "sandbox",
    }
    ...
    labels["augments.adk.io/pss"] = options.pod_security_standard
    ...
    labels["augments.sandbox/pod"] = pod_name
...
class K8sPodSandboxClient(BaseSandboxClient[K8sSandboxClientOptions]):
    ...
    async def create(
        ...
        *,
        ...
        pod_name = f"augments-sandbox-{uuid.uuid4().hex[:12]}"
        pod_body = _build_pod_manifest(options, pod_name=pod_name)
        if manifest is not None:
            from augments.adk.sandbox.policy import apply_mounts_to_k8s_pod
