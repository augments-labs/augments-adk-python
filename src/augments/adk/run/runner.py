...
from augments.adk.hooks.hooks import RunHooks, compose_run_hooks
from augments.adk.run.config import DEFAULT_MAX_TURNS, DEFAULT_RUN_CONFIG, RunConfig
from augments.adk.run.context import RunContext, TContext
...
from augments.adk.run.loop import run_agent_loop, run_agent_loop_streamed
...
from augments.adk.run.stream import CancelMode, HookEventKind, HookLifecycleEvent, RunResultStreaming
from augments.adk.tracing import agent_span
from augments.adk.tracing.spans import swarm_span
...
if TYPE_CHECKING:
    ...
    from augments.adk.flows.config import FlowConfig
    from augments.adk.flows.flow import Flow
    from augments.adk.flows.result import FlowRunResult, FlowRunResultStreaming
    from augments.adk.flows.worker_backend import FlowWorkerBackend
    from augments.adk.graphs.checkpointer import Checkpointer
    from augments.adk.graphs.graph import Graph
    from augments.adk.graphs.hooks import GraphHooks, HookProvider
    ...
    from augments.adk.run.profile import RunnerProfile
    from augments.adk.run.types import UserPrompt
    from augments.adk.swarms.checkpointer import SwarmCheckpointer
    from augments.adk.swarms.interrupt import SwarmResume
    from augments.adk.swarms.result import SwarmRunResult, SwarmRunResultStreaming
    ...
    from augments.adk.tasks.task_group import TaskGroup, TaskGroupResult
    from augments.adk.tasks.task_output import TaskOutput
    from augments.adk.tasks.task_pipeline import TaskPipeline, TaskPipelineResult
    from augments.adk.tasks.task_pipeline_state import TaskPipelineState
    from augments.adk.tracing.spans import AgentSpanData, Span
    ...
    from augments.adk.types.input.llm_input_text import LLMInputText
...
def wrap_hooks_with_verbose(
    ...
    if config.verbose is not None and config.verbose.enabled:
        from augments.adk.verbose.hooks import VerboseHooks, find_verbose_hooks
...
def _activate_mcp_run_hooks_bridge(hooks: RunHooks[Any], ctx: Any) -> None:
    ...
    """
    try:
        from augments.adk.mcp.run_hooks_bridge import active_run_context, active_run_hooks
    except ImportError:
        return
...
async def _dispose_agent_toolsets(agent: Agent) -> None:
    ...
    """
    from augments.adk.tools.toolsets import Toolset
...
async def _graph_run_bracket(
    *,
    ...
    """
    from augments.adk.verbose.hooks import emit_task_end, emit_task_start
...
class Runner:
    ...
    async def arun_swarm(
        ...
        *,
        ...
        """
        # Deferred import: swarm_loop pulls in augments.adk.swarms.* which
        # transitively imports from augments.adk.handoffs.handoff. Hoisting
        ...
        from augments.adk.session.session_event import (
            SessionEvent,
            create_session_event,
        )
        ...
        *,
        ...
        if regenerated and initial_state is not None:
            logger.warning(
                ...
                "work — the new root span starts a fresh augments.swarm.id."
            )
        ...
        *,
        ...
        if loaded_state.swarm_id is not None:
            ...
            logger.warning(
                ...
                "starts a fresh augments.swarm.id."
            )
    ...
    async def arun_task_pipeline(
        ...
        *,
        ...
        Returns:
            ...
            output.
        """
        from augments.adk.tasks.task_pipeline import TaskPipelineResult
    ...
    async def arun_task_group(
        ...
        *,
        ...
        Returns:
            ...
            ``LLMUsage.__add__``.
        """
        from augments.adk.tasks.task_group import TaskGroupResult
        ...
        *,
        ...
        Raises:
            FlowCheckpointNotFoundError: When ``checkpoint_id`` is not
                found in ``backend``.
        """
        from augments.adk.flows.exceptions import FlowCheckpointNotFoundError
    ...
    async def arun_flow_for_each(
        ...
        *,
        ...
        - Per-item ``AugmentsError`` failures land on
    ...
    async def _run_flow_batch_concurrent(
        ...
        *,
        ...
        # `_run_one_in_batch` absorbs every per-item AugmentsError into a
    ...
    async def _run_one_in_batch(
        ...
        *,
        ...
        ``AugmentsError`` subclasses that can still bubble out
        (e.g. ``FlowDefinitionError`` from ``encode_state`` on
        an unsupported state type, or a future ``AugmentsError``
        ...
        from augments.adk.flows.result import FlowRunResult
...
def _compose_task_prompt(
    ...
    """
    from augments.adk.tasks.task import Task, TaskDependency
    from augments.adk.tasks.task_input_data import TaskInputData
...
def _encode_flow_state(flow: Any) -> str:
    ...
    Routes through ``augments.adk.flows.executor.encode_state`` so
    the supported state shapes (Pydantic ``BaseModel`` /
    ``@dataclass``) and the loud-failure semantics for everything else
    stay in one place.
    """
    from augments.adk.flows.executor import encode_state
