...
from augments.adk.exceptions import AugmentsError, NoRoutingCandidateError, UsageLimitExceeded
...
from augments.adk.run.cost import check_request_limit_before_call, check_usage_limits
from augments.adk.types.input import LLMInputContentItem
from augments.adk.verbose.run_bridge import fire_stream_chunk
...
if TYPE_CHECKING:
    ...
    from augments.adk.llms import LLM
    ...
    from augments.adk.run.tools_executor import FunctionToolFailureCounts
    ...
    from augments.adk.tools.local.apply_patch_tool import ApplyPatchTool
    from augments.adk.tools.local.shell_tool import ShellTool
    from augments.adk.types.responses.llm_response import LLMResponse
    from augments.adk.types.tools import ToolChoice
...
def _current_period_key(period: BudgetPeriod) -> str:
    ...
    from augments.adk.budgets import period_key
...
def resolve_model_name(agent: Agent, config: RunConfig) -> str:
    ...
    """
    from augments.adk.llms import LLM as _LLM
...
def resolve_output_schema(agent: Agent) -> AgentOutputSchemaBase | None:
    ...
    from augments.adk.schemas import AgentOutputSchemaBase as _Base
...
async def enforce_tenant_budget(
    ...
    **closed** by default (``RunConfig.ledger_fail_open=False``): the period is
    ...
    from augments.adk.verbose.hooks import emit_budget_exceeded
...
async def build_tools(
    ...
    from augments.adk.context.context_config import CacheStrategy
    from augments.adk.tools.builtin.builtin_tool import BuiltinTool, ExecutableBuiltinTool
    from augments.adk.tools.function_tool import FunctionTool
    ...
    if len(conflicts) > 0:
        from augments.adk.exceptions import ToolsetNameConflictError
    ...
    if agent.tools:
        ...
        for candidate in all_tools:
            ...
            if isinstance(tool, FunctionTool):
                ...
                from augments.adk.tools.local.apply_patch_tool import ApplyPatchTool as _ApplyPatchTool
                from augments.adk.tools.local.shell_tool import ShellTool as _ShellTool
    ...
    if agent.skills:
        from augments.adk.skills.skill_rendering import check_skill_enabled
...
async def call_llm(
    ...
    from augments.adk.llms.llm_middleware import compose_llm_middleware
...
def _is_routing_retryable(exc: Exception) -> bool:
    ...
    Framework errors (``AugmentsError`` — budget exceeded, guardrail trips,
    ...
    """
    return not isinstance(exc, AugmentsError)
...
async def _record_rejected_routed_response(
    *,
    ...
    from augments.adk.verbose.hooks import emit_budget_exceeded, emit_usage_recorded
...
async def call_llm_with_routing(
    ...
    Escalates on non-``AugmentsError`` exceptions, ``router.should_escalate``
    returning ``True``, or output-schema validation failure. ``AugmentsError``
...
async def call_llm_streamed(
    ...
    from augments.adk.run.stream import RawResponseStreamEvent
...
def find_agent_by_name(root_agent: Agent, name: str) -> Agent | None:
    ...
    while queue:
        ...
        from augments.adk.handoffs.handoff import Handoff as _Handoff
