...
Example::
    ...
    import dataclasses
    from augments.adk.types.items import UserItem
...
class RunConfig:
    ...
    quotas, spans (``augments.tenant.id``), the ``tenant`` metric dimension,
    ...
    (OTel emits them as ``augments.metadata.<key>`` span attributes). Do
    ...
    **Safety-net semantics**: this is the "absolute safety net" for
    ...
    Example::
        ...
        from augments.adk.agents import AgentGuardrails
    ...
    ``ModelRefusalError.__mro__`` → ``[ModelRefusalError, AugmentsError, …]``
