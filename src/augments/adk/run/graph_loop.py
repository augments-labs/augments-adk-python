...
from augments.adk.graphs.hooks import HookProvider, HookRegistry
...
from augments.adk.run.node_reliability import resolve_node_reliability, run_node_with_reliability
from augments.adk.run.state import RunState
from augments.adk.run.stream import CancelMode
from augments.adk.tracing.spans import Span, graph_node_span, graph_span, graph_superstep_span
...
def _generate_thread_id() -> str:
    ...
    Matches ``augments.adk.graphs.graph._generate_graph_id`` in format
...
async def _dispatch_node(
    *,
    ...
    # augments.graph.node.status. InterruptException is a cooperative
    ...
    # augments.graph.node.resume_attempt; original firings leave the
...
async def run_graph_loop(
    *,
    ...
    with graph_span(graph_id=graph.id, entry=graph.entry) as graph_tracing_span:
        ...
        # the OTel attribute mapping surfaces augments.graph.status and
        # augments.graph.supersteps_total. Same cast pattern as the
