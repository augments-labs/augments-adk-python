...
* ``augments.adk.run.runner.Runner`` is execution for local
...
from augments.adk.a2a.a2a_agent import A2AAgent, A2ARunResult, A2AStreamEvent
...
def _check_a2a_agent(agent: object, method_name: str) -> A2AAgent:
    ...
    if not isinstance(agent, A2AAgent):
        raise TypeError(
            ...
            f"primitives, use augments.adk.run.runner.Runner."
        )
    return agent
...
class A2ARunner:
    ...
    .. important::
       ...
       ``augments.adk.run.runner.Runner`` for those.
