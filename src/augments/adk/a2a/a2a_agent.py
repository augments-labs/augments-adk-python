...
``A2AAgent`` is a sibling of ``augments.adk.agents.Agent`` — both
extend ``augments.adk.agents.BaseAgent``. The split signals to a
...
peer flows through ``augments.adk.a2a.a2a_runner.A2ARunner``, the
sibling of ``augments.adk.run.runner.Runner``. ``A2ARunner``
...
try:
    ...
    if ie.name is not None and ie.name != "a2a" and not ie.name.startswith("a2a."):
        ...
        raise
    raise ImportError(
        "Please install the 'a2a' extra to use A2A protocol support. Run: pip install 'augments-adk[a2a]'",
...
from augments.adk.a2a.a2a_client import (
...
class A2AAgent(BaseAgent[Any]):
    ...
    ``augments.adk.a2a.a2a_runner.A2ARunner``. To wrap the agent as
    ...
    def _build_tool_on_invoke(self) -> Callable[[ToolContext, str], Awaitable[str]]:
        ...
        from augments.adk.a2a.a2a_runner import A2ARunner
