...
SDK, no new runtime dependency**. Drives the `augments deploy` command
group (`cli/deploy.py`).
...
| 3 | The generated image binds `0.0.0.0:$PORT` explicitly in the CMD | Meets the universal container contract; `augments serve` keeps its secure `127.0.0.1` default for local runs. |
| 4 | `requirements.txt` is the install seam | The Dockerfile installs `augments-adk[...]` from whatever the operator's `requirements.txt` points at — PyPI by default, or a pin / private index / vendored wheel / VCS URL when the build must not reach PyPI. |
