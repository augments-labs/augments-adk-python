"""Augments Restate service helpers and human-in-the-loop support.

Provides ``AugmentsRestateService`` — a mixin/base class with helpers
...
class RestateHumanReply:
    ...
    Constructed by ``AugmentsRestateService.wait_for_human_reply`` from
...
class AugmentsRestateService:
    """Mixin providing common Restate service helpers for Augments workflows.
    ...
    Example::
        ...
        helpers = AugmentsRestateService()
    ...
    async def wait_for_human_reply(
        ...
        logger.info(
            "AugmentsRestateService.wait_for_human_reply: awaiting promise %r",
            promise_name,
        )
        raw: dict[str, Any] = await ctx.promise(promise_name).value()
        logger.debug(
            "AugmentsRestateService.wait_for_human_reply: promise %r resolved, node_id=%r",
            promise_name,
            raw.get("node_id"),
        )
