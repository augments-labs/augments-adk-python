"""Worker wiring plugin for Augments ADK Temporal integration.

``AugmentsTemporalPlugin`` implements BOTH of temporalio's plugin
surfaces — ``temporalio.client.Plugin`` and ``temporalio.worker.Plugin``
— so one instance wires the Augments data converter into the client and
...
from augments.adk.workflows.temporal.activity import register_model as activity_register_model
from augments.adk.workflows.temporal.determinism import (
    DEFAULT_PASSTHROUGH_MODULES,
    build_sandbox_restrictions,
)
from augments.adk.workflows.temporal.serialization import build_augments_data_converter
...
class AugmentsTemporalPlugin(ClientPlugin, WorkerPlugin):
    """Client + worker plugin wiring Augments Temporal workflows.

    On the client side, installs the Augments data converter. On the
    worker side, installs a sandboxed workflow runner whose passthrough
    modules cover the Augments ADK and its dependencies, and syncs the
    ...
    def __post_init__(self) -> None:
        ...
        logger.info(
            "AugmentsTemporalPlugin initialized with %d passthrough modules",
    ...
    def register_model(self, name: str, llm: LLM) -> None:
        ...
        logger.info("AugmentsTemporalPlugin registered model %r", name)

    # -- temporalio.client.Plugin surface ---------------------------------

    @override
    def configure_client(self, config: ClientConfig) -> ClientConfig:
        """Install the Augments data converter on the client.

        The converter is a client-level setting in temporalio; workers
        and workflow handles created from the client inherit it.
        """
        config["data_converter"] = build_augments_data_converter()
        logger.debug("AugmentsTemporalPlugin: data converter installed on client config")
        return config
    ...
    def configure_worker(self, config: WorkerConfig) -> WorkerConfig:
        ...
        logger.debug("AugmentsTemporalPlugin: sandboxed workflow runner installed on worker config")
        return config
