"""Base workflow class and HITL payload types for Augments Temporal workflows.

Provides ``AugmentsWorkflow`` — a ``defn``-decorated
...
try:
    ...
    class AugmentsWorkflow:
        ...
        The class attribute ``__augments_agents__`` is a sequence of agent
        instances that the workflow orchestrates.  Subclasses populate it at
        class definition time.

        Attributes:
            __augments_agents__: Agent instances orchestrated by this workflow.
                Defaults to an empty tuple; subclasses override at class level.
        """

        __augments_agents__: Sequence[Any] = ()
