...
Bridge layer that makes any Augments `Agent`, `Swarm`, `Graph`, or `Flow` run
...
| `workflow.py` | `AugmentsWorkflow` base class with HITL signals/queries/updates; `HumanReply`, `ToolApprovalDecision` |
| `plugin.py` | `AugmentsTemporalPlugin` — bundles sandbox + data-converter kwargs for the worker |
...
| `serialization.py` | `build_augments_data_converter` — custom Temporal `DataConverter` |
...
| `service.py` | `AugmentsRestateService` mixin + `RestateHumanReply` (HITL over promises) |
...
| 3 | **`AugmentsWorkflow` base with HITL signals/queries/updates** | Maps Temporal's signal → `send_human_reply` (enqueue), query → `get_state` (snapshot), update → `approve_tool_call` (decision). `InterruptException` raised by graph HITL nodes maps to `wait_for_condition` + `consume_replies` in the subclass `run()`. |
...
| 7 | **No `TemporalRunner` — `Runner.arun()` called inside `@workflow.run`** | Avoids a parallel runner hierarchy. Concrete subclasses of `AugmentsWorkflow` import and call `Runner` directly; the framework adds no hidden entry point. Matches the "Agent = config, Runner = execution" invariant. |
