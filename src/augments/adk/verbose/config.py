...
``"auto"`` defers to ``augments.adk.verbose.mode.resolve_mode``
...
# Enrichment beyond CrewAI — HITL (first-class in Augments, limited in
...
def _default_styles() -> dict[str, EventStyle]:
    ...
    return {
        ...
        # HITL (enrichment — first-class in Augments). Panel titles
...
class VerboseConfig:
    ...
    emits nothing. See ``augments.adk.verbose.mode.resolve_mode``
