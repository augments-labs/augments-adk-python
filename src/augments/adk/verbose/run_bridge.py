...
(``augments.adk.run.llm_calls.call_llm_streamed``) intentionally
...
``augments.adk.mcp.run_hooks_bridge``.
"""
...
def fire_stream_chunk(accumulated_text: str, call_type: str) -> None:
    ...
    if hooks is None or agent is None:
        return
    try:
        from augments.adk.verbose.hooks import emit_stream_chunk
