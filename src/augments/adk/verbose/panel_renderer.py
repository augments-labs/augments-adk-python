...
* **Task boundary** (``render_task_start`` / ``render_task_end``) —
  ...
  ``augments.adk.run.runner.Runner.arun`` entry / exit.
* **Live streaming** (``open_stream_panel`` / ``update_stream_panel`` /
  ...
  ``augments.adk.verbose.run_bridge`` ``ContextVar`` bridge.
...
from augments.adk.verbose.renderer import redact_secrets
from augments.adk.verbose.state import BlockKey, BlockNode, RunTree
...
class PanelRenderer:
    ...
    def resume_live_updates(self) -> None:
        ...
        (``augments.adk.run.llm_calls``). The next chunk emission will
...
def format_tool_payload(tool_input: Any, tool_output: Any = None) -> str:
    ...
    redacted via ``augments.adk.verbose.renderer.redact_secrets``
    ...
    """
    from augments.adk.verbose.renderer import format_payload
