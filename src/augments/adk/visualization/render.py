...
- ``pip install 'augments-adk[viz]'`` — adds the ``graphviz`` Python
  package, which shells out to the local ``dot`` CLI. Used by
  ``render_dot`` to produce SVG.
- ``pip install 'augments-adk[mermaid]'`` — adds ``mermaid-py``, which
...
def render_dot(dot_source: str, out_path: Path) -> RenderOutcome:
    ...
    try:
        ...
        logger.info("Install 'augments-adk[viz]' to render DOT.")
        return "skipped"
    try:
...
def render_mermaid(mermaid_source: str, out_path: Path) -> RenderOutcome:
    ...
    try:
        ...
        logger.info("Install 'augments-adk[mermaid]' to render Mermaid.")
        return "skipped"
    try:
