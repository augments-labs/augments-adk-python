...
1. **Readability first.** A graph definition should read top-to-bottom
   ...
   - Strands: ``GraphBuilder().add_node(agent, "x").add_edge("x", "y")``
     ...
     versions.
   - Augments (this file): ``Graph.new("x").node("a", agent).node("b", agent).pipe("a", "b").entry("a").terminal("b").compile()``.
...
from augments.adk.graphs.merge import DEFAULT_MERGE, MergeFn
from augments.adk.graphs.node import EdgeCondition, GraphEdge, GraphNode, NodeErrorHandlerFn
        ...
        *,
        ...
        Args:
            ...
            merge: Optional override for the default
                ``augments.adk.graphs.merge`` strategy. Applied when
