...
buckets. The ``augments.adk.flows.executor.FlowExecutor`` reads it
...
if TYPE_CHECKING:
    ...
    from augments.adk.flows.step_cache_policy import FlowStepCachePolicy
    from augments.adk.flows.step_context import FlowStepGate
    from augments.adk.flows.step_guardrails import FlowStepGuardrails
    from augments.adk.flows.step_rate_limit import FlowStepRateLimit
...
class FlowStep:
    ...
    Built by ``augments.adk.flows.decorators.flow_start`` /
    ``augments.adk.flows.decorators.flow_listen`` /
    ``augments.adk.flows.decorators.flow_router``. The wrapper is:
    ...
    async def check_enabled(self, ctx: Any) -> bool:
        ...
        Args:
            ctx: The ``FlowStepContext`` for the step about to
                ...
                with ``augments.adk.flows.step_context``; callers
...
async def _evaluate_gate(gate: Any, ctx: Any, *, default: bool) -> bool:
    ...
    Anything else raises ``augments.adk.flows.exceptions.FlowDefinitionError``
