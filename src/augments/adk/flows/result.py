...
(``augments.adk.types.run.run_result.RunResult``):
...
if TYPE_CHECKING:
    ...
    from augments.adk.flows.deferred import FlowDeferredStep
    from augments.adk.flows.events import FlowEvent
...
class FlowRunResult[StateT]:
    ...
    Attributes:
        ...
        per_step_usage: Mapping from step name to LLM usage delta for
            ...
            analogous to ``augments.adk.graphs.result.GraphRunResult.per_node_usage``.
    ...
    def requires_action(self) -> bool:
        ...
        Exact mirror of
        ``augments.adk.types.run.run_result.RunResult.requires_action``
...
class FlowRunResultStreaming[StateT]:
    ...
    pattern as ``augments.adk.run.stream.RunResultStreaming``).
    ...
    def push_event(self, event: FlowEvent) -> None:
        ...
        Public accessor used by ``augments.adk.run.runner.Runner`` to
        forward executor events into the queue without reaching into
        the private ``_event_queue`` attribute. Same encapsulation
        pattern as ``augments.adk.run.stream.RunResultStreaming.put_event``.
    ...
    def set_producer_task(self, task: asyncio.Task[None]) -> None:
        ...
        ``augments.adk.run.stream.RunResultStreaming.set_run_task``.
    ...
    def set_deferred_run_impl(self, impl: Callable[[], Coroutine[Any, Any, None]]) -> None:
        ...
        Used by ``augments.adk.run.runner.Runner.arun_flow_streamed``
        ...
        ``augments.adk.run.stream.RunResultStreaming.set_deferred_run_impl``.
