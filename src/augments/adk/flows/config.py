...
This config is a sibling to ``augments.adk.run.config.RunConfig``. Both
...
- ``"halt"`` — surfaces the exception immediately as
  ``augments.adk.flows.exceptions.FlowStepError``. Cost-conservative
...
- ``"route_to_error_handler"`` — looks up a listener decorated as
  ...
  literal; see ``augments.adk.flows.triggers.FLOW_ERROR_TRIGGER``)
...
class FlowConfig:
    ...
    Attributes:
        max_steps: Hard cap on the total number of step invocations
            ...
            cap is hit, ``augments.adk.flows.exceptions.FlowMaxStepsExceeded``
        ...
        max_total_tokens: Optional cumulative cap on tokens consumed by
            ...
            to ``augments.adk.run.config.RunConfig.max_total_tokens``.
        max_listeners_per_step: Maximum number of downstream methods
            ...
            ``augments.adk.flows.exceptions.FlowDefinitionError`` if
            exceeded.
