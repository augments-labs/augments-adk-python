...
``augments.adk.flows.decorators``.

A ``Flow`` is configuration; the Runner executes it. So ``Flow``
carries NO ``run()`` / ``arun()`` method. Execution lives on
``augments.adk.run.runner.Runner.arun_flow`` /
``augments.adk.run.runner.Runner.arun_flow_streamed`` /
``augments.adk.run.runner.Runner.arun_flow_from_checkpoint``.
...
2. Collects ``@flow_start`` / ``@flow_listen`` / ``@flow_router`` registrations into a
   frozen ``augments.adk.flows.registry.FlowStepRegistry``.
...
from augments.adk.flows.registry import FlowStepRegistry, TriggerSpec

if TYPE_CHECKING:
    from augments.adk.flows.deferred import FlowApprovalDecision
    from augments.adk.flows.definition import FlowDefinition
    from augments.adk.run.context import RunContext
    from augments.adk.visualization.dot import DotRankdir
    from augments.adk.visualization.mermaid import MermaidDirection
...
class Flow[StateT](metaclass=FlowMeta):
    ...
    Example::
        ...
        from augments.adk.flows import Flow, flow_start, flow_listen, flow_router
    ...
    def to_mermaid(self, *, direction: MermaidDirection = "LR") -> str:
        ...
        ``augments.adk.visualization.flow_to_mermaid``. Pure function:
        ...
        """
        from augments.adk.visualization.mermaid import flow_to_mermaid
    ...
    def to_dot(self, *, rankdir: DotRankdir = "LR") -> str:
        ...
        Thin ergonomic wrapper around
        ``augments.adk.visualization.flow_to_dot``. Pure function:
        ...
        """
        from augments.adk.visualization.dot import flow_to_dot
