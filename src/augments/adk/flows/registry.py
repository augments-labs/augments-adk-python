...
1. ``FlowStepRegistry`` — built ONCE by
   ``augments.adk.flows.flow.FlowMeta`` at class-creation time, from
...
class FlowStepRegistry:
    ...
    Built by ``augments.adk.flows.flow.FlowMeta`` walking ``cls.__dict__``
    once at class-creation time. The metaclass collects function attributes
    set by the decorators in ``augments.adk.flows.decorators``, validates
