...
All inherit from ``augments.adk.exceptions.AugmentsError`` so the
framework-wide exception hierarchy is preserved. Module-scoped rather than
in the central ``augments.adk.exceptions`` file because they are tightly
coupled to Flow internals — same precedent as
``augments.adk.graphs.interrupt.InterruptException``.
"""

from __future__ import annotations

from augments.adk.exceptions.exceptions import AugmentsError, UserError
...
class FlowMaxStepsExceeded(AugmentsError):
    """Raised by ``FlowExecutor`` when the configured ``max_steps`` cap is hit.

    Distinct from ``augments.adk.exceptions.MaxTurnsExceeded`` (which
...
class FlowStepError(AugmentsError):
...
class FlowStepDeferred(AugmentsError):
...
class FlowStepRateLimitExceeded(AugmentsError):
...
class FlowStepGovernanceError(AugmentsError):
...
class FlowStepGuardrailTripped(AugmentsError):
    ...
    """Routed verdict message (renamed to avoid LSP clash with ``AugmentsError.message``)."""
...
class FlowAgentDeferred(AugmentsError):
...
class FlowStepSkipped(AugmentsError):
...
class FlowStepRejected(AugmentsError):
    ...
    ``AugmentsError.message`` attribute (which is typed
    ``str``, never ``None``).
    """
