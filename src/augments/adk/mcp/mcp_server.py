...
from augments.adk.mcp.notifications import make_message_handler
...
try:
    ...
    raise ImportError(
        "Please install the 'mcp' extra to use MCPServer. You can use the `mcp` optional group - pip install augments-adk[mcp]"
    ) from ie
...
class MCPServer(abc.ABC):
    ...
    ``augments.adk.types``; conversion happens in ``conversion.py``.
