...
handed to ``augments.adk.tracing.set_tracer``. It reads
    ...
    *,
    ...
    Args:
        ...
        convention: Span-attribute vocabulary the returned tracer emits.
            ...
            ``augments.*``). Pass ``TracingConvention.OPENINFERENCE`` to
...
def setup_otel_from_env(
    ...
    * ``OTEL_SERVICE_NAME`` — value for the ``service.name`` resource
      attribute.  When absent the framework defaults to ``"augments-adk"``.
    ...
    service_name = os.environ.get("OTEL_SERVICE_NAME", "augments-adk")
    logger.debug(
