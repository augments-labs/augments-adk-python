...
from augments.adk.tracing.openinference import conventions as oi
...
def _flatten_attributes(exported: dict[str, Any], *, prefix: str = "") -> dict[str, Any]:
    ...
    Separator semantics::
        ...
        _flatten_attributes({"a": 1}, prefix="augments")  → {"augments.a": 1}
...
def _agent_attrs(data: AgentSpanData) -> dict[str, Any]:
    attrs: dict[str, Any] = {
        "augments.agent.name": data.name,
    }
    if data.handoffs is not None and len(data.handoffs) > 0:
        attrs["augments.agent.handoffs"] = list(data.handoffs)
    if data.tools is not None and len(data.tools) > 0:
        attrs["augments.agent.tools"] = list(data.tools)
    if data.output_type is not None:
        attrs["augments.agent.output_type"] = data.output_type
    for k, v in data.metadata.items():
        attrs.update(_flatten_attributes({k: v}, prefix="augments.metadata"))
    if data.tenant_id is not None:
        attrs["augments.tenant.id"] = data.tenant_id
    return attrs
...
def _function_attrs(
    ...
    *,
    ...
    attrs: dict[str, Any] = {
        "augments.tool.name": data.name,
    }
    if data.input is not None:
        attrs["augments.tool.input"] = data.input if record_full else _redact_and_truncate(data.input, max_chars)
    if data.output is not None:
        raw_output = str(data.output)
        attrs["augments.tool.output"] = raw_output if record_full else _redact_and_truncate(raw_output, max_chars)
    if data.mcp_data is not None:
        attrs.update(_flatten_attributes(data.mcp_data, prefix="augments.mcp"))
    if data.a2a_data is not None:
        ...
        attrs.update(_flatten_attributes(a2a_safe, prefix="augments.a2a"))
    return attrs


def _generation_attrs(data: GenerationSpanData) -> dict[str, Any]:
    attrs: dict[str, Any] = {"gen_ai.system": "augments"}
...
def _handoff_attrs(data: HandoffSpanData) -> dict[str, Any]:
    ...
    if data.from_agent is not None:
        attrs["augments.handoff.from"] = data.from_agent
    if data.to_agent is not None:
        attrs["augments.handoff.to"] = data.to_agent
    return attrs


def _guardrail_attrs(data: GuardrailSpanData) -> dict[str, Any]:
    return {
        "augments.guardrail.name": data.name,
        "augments.guardrail.triggered": data.triggered,
    }


def _custom_attrs(data: CustomSpanData) -> dict[str, Any]:
    attrs: dict[str, Any] = {"augments.span.name": data.name}
    attrs.update(_flatten_attributes(data.data, prefix="augments.custom"))
    return attrs
...
def _graph_attrs(data: CustomSpanData) -> dict[str, Any]:
    ...
    the ``augments.graph.*`` namespace. ``None``-valued fields are
    ...
    if graph_id is not None:
        attrs["augments.graph.id"] = graph_id
    entry = payload.get("entry")
    if entry is not None:
        attrs["augments.graph.entry"] = entry
    status = payload.get("status")
    if status is not None:
        attrs["augments.graph.status"] = status
    supersteps_total = payload.get("supersteps_total")
    if supersteps_total is not None:
        attrs["augments.graph.supersteps_total"] = supersteps_total
    return attrs
...
def _graph_superstep_attrs(data: CustomSpanData) -> dict[str, Any]:
    ...
    ``augments.graph.superstep.*``. The parent ``augments.graph.id`` is
...
def _graph_node_attrs(data: CustomSpanData) -> dict[str, Any]:
    ...
    ``augments.graph.node.*``. ``resume_attempt`` only appears when the
    ...
    if node_name is not None:
        attrs["augments.graph.node.name"] = node_name
    attempts = payload.get("attempts")
    if attempts is not None:
        attrs["augments.graph.node.attempts"] = attempts
    status = payload.get("status")
    if status is not None:
        attrs["augments.graph.node.status"] = status
    duration_ms = payload.get("duration_ms")
    if duration_ms is not None:
        attrs["augments.graph.node.duration_ms"] = duration_ms
    resume_attempt = payload.get("resume_attempt")
    if resume_attempt is not None:
        attrs["augments.graph.node.resume_attempt"] = resume_attempt
    return attrs
...
def _swarm_attrs(data: CustomSpanData) -> dict[str, Any]:
    ...
    the ``augments.swarm.*`` namespace. ``None``-valued fields are
    omitted (OTel rejects ``None`` attribute values).
    """
    payload = data.data
    attrs: dict[str, Any] = {}
    ...
    if swarm_id is not None:
        attrs["augments.swarm.id"] = swarm_id
    entry = payload.get("entry")
    if entry is not None:
        attrs["augments.swarm.entry"] = entry
    status = payload.get("status")
    if status is not None:
        attrs["augments.swarm.status"] = status
    turns_total = payload.get("turns_total")
    if turns_total is not None:
        attrs["augments.swarm.turns_total"] = turns_total
    return attrs
...
def _swarm_turn_attrs(data: CustomSpanData) -> dict[str, Any]:
    ...
    ``augments.swarm.turn.*``. The parent ``augments.swarm.id`` is repeated
    ...
    index = payload.get("index")
    if index is not None:
        attrs["augments.swarm.turn.index"] = index
    member = payload.get("member")
    if member is not None:
        attrs["augments.swarm.turn.member"] = member
    status = payload.get("status")
    if status is not None:
        attrs["augments.swarm.turn.status"] = status
    duration_ms = payload.get("duration_ms")
    if duration_ms is not None:
        attrs["augments.swarm.turn.duration_ms"] = duration_ms
    resume_attempt = payload.get("resume_attempt")
    if resume_attempt is not None:
        attrs["augments.swarm.turn.resume_attempt"] = resume_attempt
    return attrs
        ...
        *,
        ...
        Args:
            ...
            convention: Span-attribute vocabulary to emit. Defaults to
                ...
                ``augments.*``). Pass ``TracingConvention.OPENINFERENCE``
