...
- **No memory-layer leakage** into shipped artifacts: never the literal
  `.agents/`, `.claude/`, `AGENTS.md`, `CLAUDE.md`, a playbook basename, or
  a rule title in shipped code/prose/comments. Inline the claim in its own
  words instead. Detail: `no-memory-in-shipped-code.md`.
