...
from augments.adk.tracing.logging import EVENT_AGENT_TURN_START, log_event


def test_log_event_emits_structured_fields(caplog):
    logger = logging.getLogger("augments.test")
    with caplog.at_level(logging.INFO, logger="augments.test"):
...
def test_log_event_respects_level(caplog):
    logger = logging.getLogger("augments.test2")
    with caplog.at_level(logging.WARNING, logger="augments.test2"):
...
def test_log_event_forwards_level(caplog):
    logger = logging.getLogger("augments.test3")
    with caplog.at_level(logging.WARNING, logger="augments.test3"):
